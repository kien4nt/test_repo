    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     String ID = request.getParameter("pid");
      ...
      BookDAO bookDAO = new BookDAO();
        ...
        CategroryDAO CategroryDAO = new CategroryDAO();
        int bookID = Integer.parseInt(ID);

        // Fetch book, authors, and publisher details
        List<Author> authors = authorDAO.getbookbyAuthor(ID);
        Book book = bookDAO.getbookbyid(bookID);
        Publisher publisher = publisherDAO.getPublisherByBookID(bookID);
           List<Category> categories = CategroryDAO.getCategoriesByBookID(ID);


        request.setAttribute("Chitiet", book);
        request.setAttribute("Chitiet1", authors);
        request.setAttribute("Chitiet2", publisher);  // Publisher information
        request.setAttribute("Chitiet3", categories);  // Publisher information
